...
export const computePool = (declarations: RoundDeclarations): number =>
  162 + declarations.tierces * 20 + declarations.poltinas * 50 + (declarations.hasBela ? 20 : 0);

export const computeBaitPenalty = (baitCount: number): number => {
...
export const resolveRound = (
  ...
  if (declaringScore === opponentScore) {
    isDraw = true;
    // nobody gets points — full pool carries over to next round
...
export const useGameStore = defineStore('game', () => {
  ...
  const setupGame = (names: [string, string], format: GameFormat) => {
  ...
  const addRound = (form: RoundFormState) => {
    ...
    carryOver.value = resolved.isDraw ? pool : 0;
    saveToStorage();
  };
